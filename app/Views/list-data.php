	...
	<style {csp-style-nonce}>
		...
		*:focus {
		...
		input[type=text], input[type=password] {
		  border: none;
		  border-bottom: 2px solid rgba(221, 72, 20, .8);
		  width:50%;
		  padding: 12px 20px;
  			margin: 8px 0;
...
<script type="text/javascript">
	$.getJSON('https://jsonplaceholder.typicode.com/users', function(data) {
        $.each(data, function(index) {
            ...
            $('<tr>').html("<td>" + data[index].name + "</td><td>" + data[index].username + "</td><td>" + data[index].phone + "</td><td>" + data[index]["company"].name + "</td>").appendTo('#listd');
        });
    });
</script>
<!-- -->
